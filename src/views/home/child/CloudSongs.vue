<template>
    <div v-if="this.res">
        <div class="dark:border-[#2d2f36] w-[100vw] px-[4vw] border-t-[1px] border-[#EBEDF2]">
            <div class="flex justify-between items-center h-[12vw]">
                <div class="flex items-center"><span
                        class="dark:text-[#e9ebf2] text-[#374d5b] text-[4vw] font-[800]">新歌新碟\数字专辑</span> <svg
                    ...
                    </svg></div> <svg  @click="showPopup" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                    aria-hidden="true" role="img" width="8" height="8" viewBox="0 0 24 24"
                ...
                <div class="dark:bg-[#272727] bg-[#fff] fixed z-[11]"
                    ...
                    <div>
                        <div
                            ...
                            <p class="text-[3vw] text-[#939BA1] font-[800]">新歌新碟\数字专辑</p> <svg @click="show=false"
        ...
        <div class="w-[98vw] pt-[1vw] pl-[2vw] overflow-hidden scroll-wrapper border-[#EBEDF2]" ref="scroll">
            <div class="scroll-content flex w-[360vw] overflow-hidden">
                <ul class="w-[88vw] scroll-item pb-[3.8vw]" style="pointer-events: auto;" v-for="item,index in res" :key="index">
                    <li class="pl-[2vw] w-[88vw] scroll-content pt-[2.7vw]" v-for="item,index in item.resources" :key="index">
                        <div class="flex"><img :src="item.uiElement.image.imageUrl"
                                alt="" class="w-[14.5vw] h-[14.5vw] rounded-[10px] flex-5">
                            <div class="flex-1 pt-[2.85vw] ml-[1vw]">
                                <h4 class="dark:text-[#e3e5ec] text-[3.5vw] text-[#3E465B]">{{ item.uiElement.mainTitle.title }}</h4>
                                <p class="dark:text-[#a8aaaf] text-[2.36vw] text-[#79838F]">{{item.uiElement.subTitle ? item.uiElement.subTitle.title :'超70%人播放'}} </p>
                            </div>
                        </div>
                    </li>
                </ul>
...
export default {
    ...
    async created() {
        ...
        if(!err){
            this.res = res.data.data.blocks[4].creatives;
        }
    },
    mounted() {
        this.init()
